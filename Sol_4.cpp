...
int main()
{
	...
	while(t--){
		...
		while(!a.empty())
		{
			temp=a.front();
			a.pop();
			if(temp==z || (temp==y && temp[ind1][ind2]==kp))
			{
				while(temp != tull)
				{
					s.push(temp);
			...
			/*if(!gflag && temp==y && temp[ind1][ind2]==kp)
			{
				while(temp != tull)
				{
					s.push(temp);
